...
class OrderViewModel(application: Application) : AndroidViewModel(application) {
    ...
    // 1. Screen state track karne ke liye backing property UI State Flow
    private val _orders = MutableStateFlow<List<Order>>(emptyList())
    val orders: StateFlow<List<Order>> = _orders

    // Loading aur Error states ko manage karne ke liye (Optional par recommended)
    ...
    private fun loadOrdersFromAssets() {
        viewModelScope.launch {
            ...
            // Background IO Thread par heavy processing execute karein
            val parsedList = withContext(Dispatchers.IO) {
                try {
                    // TODO: Apni sahi JSON file ka naam dalein (e.g., "orders.json")
                    ...
                    e.printStackTrace()
                    emptyList<Order>() // Crash hone par empty fallback list bejein
                }
            }

            // Main UI Thread par data state update karein
